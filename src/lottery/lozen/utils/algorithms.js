 ...
 */
export async function createPoolAndPick(weights, power, rng) {
    const pool = [];
 ...
 * @returns {Promise<number[]>} Array of selected numbers
 */
export async function getWeightedNumbers(rng, winningNumbers = []) {
    const frequency = {};
 ...
 * @returns {Promise<number[]>} Array of selected numbers
 */
export async function getAdaptiveNumbers(rng, winningNumbers = []) {
    const weights = {};
    for (let i = 1; i <= CONFIG.TOTAL_NUMBERS; i++) {
        weights[i] = 1.0;
    }
 ...
 * @returns {Promise<number[]>} Array of selected numbers
 */
export async function getNonFrequencyNumbers(rng, winningNumbers = []) {
    const frequency = {};
    for (let i = 1; i <= CONFIG.TOTAL_NUMBERS; i++) {
        frequency[i] = 0;
    }
